/**
 **************************************************
 * @file        NTRIPServer.ino
 * @brief       Push RTCM data from a ZED-F9P base station to an NTRIP caster over WiFi.
 *
 * @details     Collects RTCM correction data from a ZED-F9P base station over
 *              I2C and streams it to RTK2Go as an NTRIP server over ESP32
 *              WiFi. Rovers can then connect to the mountpoint to receive
 *              RTCM corrections for centimetre-level RTK accuracy. A
 *              pre-registered mountpoint on RTK2Go and WiFi credentials are
 *              required in secrets.h.
 *
 * Requirements:
 * - Board:      ESP32-based Arduino board with WiFi
 * - Hardware:   u-blox ZED-F9P configured as a base station
 * - Extra:      WiFi network; registered RTK2Go mountpoint (secrets.h)
 *
 * How to use:
 * 1) Register a mountpoint at http://www.rtk2go.com/new-reservation/
 * 2) Enter your WiFi SSID, password, mountpoint name, and password
 *    in secrets.h.
 ...
 *
 * Expected output:
 * - WiFi connection and caster connection status.
 * - Bytes pushed to the caster reported periodically.
 *
