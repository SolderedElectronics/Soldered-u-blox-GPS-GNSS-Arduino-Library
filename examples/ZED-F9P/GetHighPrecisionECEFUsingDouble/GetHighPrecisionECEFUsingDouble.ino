/**
 **************************************************
 * @file        GetHighPrecisionECEFUsingDouble.ino
 * @brief       Read high-precision ECEF coordinates as 64-bit double values.
 *
 * @details     Reads the high-precision Earth-Centered, Earth-Fixed (ECEF)
 *              positional solution from the ZED-F9P using 64-bit double
 *              arithmetic to preserve centimetre-level accuracy. A warning is
 *              printed if the platform only supports 32-bit double. The
 *              position is queried once per second.
 *
 * Requirements:
 ...
 *
 * Expected output:
 * - X, Y, Z ECEF coordinates in centimetres printed once per second.
 *
 * Notes:
