/**
 **************************************************
 * @file        NTRIPClient_With_GGA_Callback.ino
 * @brief       NTRIP client over WiFi using a NMEA callback to transmit GGA automatically.
 *
 * @details     Uses ESP32 WiFi to connect to an NTRIP caster (e.g. Skylark,
 *              RTK2Go, or Emlid Caster), receive RTCM correction data, and
 *              push it over I2C to a ZED-F9x. A NMEA callback automatically
 *              captures and broadcasts the GGA sentence to the caster every
 *              10 s. WiFi and NTRIP credentials must be placed in secrets.h.
 *              Disconnecting from the caster will cause accuracy to degrade;
 *              reconnecting will allow it to recover.
 *
 * Requirements:
 * - Board:      ESP32-based Arduino board with WiFi
 * - Hardware:   u-blox ZED-F9P
 * - Extra:      WiFi network; NTRIP caster credentials (secrets.h)
 *
 * How to use:
 * 1) Enter your WiFi SSID, password, NTRIP caster host, port, mountpoint,
 *    and credentials in secrets.h.
 * 2) Connect the ZED-F9P to the ESP32 via I2C.
 * 3) Upload the sketch.
 * 4) Open Serial Monitor at 115200 baud.
 *
 * Expected output:
 * - WiFi and NTRIP connection status.
 * - RTCM bytes received; GGA sentence transmitted automatically every 10 s.
 * - RTK fix improvement visible in the navigation output.
 *
