/**
 **************************************************
 * @file        LBand_Corrections_with_NEO-D9S.ino
 * @brief       Forward SPARTN L-Band corrections from NEO-D9S to a ZED-F9x over I2C.
 *
 * @details     Reads UBX-RXM-PMP correction data from a NEO-D9S L-Band receiver
 *              via a callback and pushes each PMP message over I2C to a ZED-F9x.
 *              The ZED-F9x uses the SPARTN corrections to achieve centimetre-level
 *              RTK accuracy without a separate NTRIP base station. A NAV PVT
 *              callback prints position and fix type to show correction effect.
 *              SPARTN decryption keys must be obtained from a Thingstream
 *              PointPerfect account and placed in secrets.h.
 *
 * Requirements:
 * - Board:      Any Arduino-compatible board with I2C
 * - Hardware:   u-blox ZED-F9P and NEO-D9S L-Band correction data receiver
 * - Extra:      Thingstream PointPerfect L-Band credentials (secrets.h)
 *
 * How to use:
 * 1) Copy your PointPerfect Current Key and Next Key into secrets.h.
 * 2) Connect both the NEO-D9S and ZED-F9P to the I2C bus.
 * 3) Upload the sketch.
 * 4) Open Serial Monitor at 115200 baud.
 *
 * Expected output:
 * - "New RXM-PMP data received. Message payload length is N Bytes. Pushing it to the GNSS..."
 * - Latitude, longitude, altitude, fix type, and carrier solution type from NAV PVT.
 *
 * Notes:
 * - Set the L-Band frequency constant for your region (US: 1556290000 Hz,
 *   EU: 1545260000 Hz).
 *
