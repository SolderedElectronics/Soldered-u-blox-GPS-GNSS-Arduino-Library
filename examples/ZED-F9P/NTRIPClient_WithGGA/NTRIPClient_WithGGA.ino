/**
 **************************************************
 * @file        NTRIPClient_WithGGA.ino
 * @brief       NTRIP client over WiFi that periodically transmits GGA sentences to the caster.
 *
 * @details     Uses ESP32 WiFi to connect to an NTRIP caster, receive RTCM
 *              correction data, and push it over I2C to a ZED-F9x. The rover's
 *              GGA sentence is extracted from incoming NMEA and broadcast to
 *              the caster every 10 s, as required by some casters for
 *              distance-based mountpoint selection. WiFi and NTRIP credentials
 *              must be placed in secrets.h.
 *
 * Requirements:
 ...
 *
 * Expected output:
 * - WiFi connection and NTRIP connection status.
 * - RTCM bytes received; GGA sentence transmitted every 10 s.
