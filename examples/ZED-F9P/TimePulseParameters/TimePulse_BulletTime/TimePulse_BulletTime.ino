/**
 **************************************************
 * @file        TimePulse_BulletTime.ino
 * @brief       Configures the TIMEPULSE (PPS) pin to output a 1 Hz pulse
 *              with an adjustable nanosecond delay for "bullet time" camera
 *              triggering.
 *
 * @details     Uses UBX-CFG-TP5 to program TIMEPULSE to produce a 0.1 s
 *              high pulse once per second, active only when the module is
 *              locked to GNSS time. The userConfigDelay field shifts the
 *              pulse by up to ±2.147 s in nanosecond steps, allowing each
 *              camera in a multi-camera array to be triggered at a different
 *              offset — replicating the "bullet time" freeze-frame effect.
 *              The configuration is saved to battery-backed memory.
 *
 * Requirements:
 ...
 * 2) Uncomment the desired userConfigDelay line for each camera.
 * 3) Upload the sketch.
 * 4) Open Serial Monitor at 115200 baud.
 *
 * Expected output:
 * - UBX_CFG_TP5 version and "Success!" (or an error message) printed at
 *   startup. No further serial output; the pulse appears on the TIMEPULSE
 *   pin.
 *
