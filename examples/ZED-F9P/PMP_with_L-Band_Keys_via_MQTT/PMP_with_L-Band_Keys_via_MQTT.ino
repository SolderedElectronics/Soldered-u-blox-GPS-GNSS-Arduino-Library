/**
 **************************************************
 * @file        PMP_with_L-Band_Keys_via_MQTT.ino
 * @brief       Fetch PointPerfect L-Band SPARTN keys via MQTT and push them to a ZED-F9x.
 *
 * @details     Uses ESP32 WiFi and MQTT over TLS to connect to the Thingstream
 *              PointPerfect broker and download the current and next L-Band
 *              dynamic decryption keys. The keys are then pushed over I2C to a
 *              ZED-F9x, enabling it to decrypt SPARTN correction data received
 *              from a NEO-D9S L-Band receiver. PMP messages from the NEO-D9S
 *              are forwarded to the ZED-F9x via a callback. Requires a valid
 *              Thingstream PointPerfect L-Band or L-Band + IP plan and client
 ...
 * 3) Connect both the NEO-D9S and ZED-F9P to the I2C bus.
 * 4) Upload the sketch.
 * 5) Open Serial Monitor at 115200 baud.
 *
 * Expected output:
 * - MQTT connection and key download status.
 * - Current and next L-Band keys printed for reference.
 * - NAV PVT position/fix type from the ZED-F9P once corrections are active.
 *
 * Notes:
