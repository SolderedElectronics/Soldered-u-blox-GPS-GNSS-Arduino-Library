/**
 **************************************************
 * @file        SendCustomCommand.ino
 * @brief       Demonstrates how to build and send a custom UBX packet and
 *              parse the response using the library's sendCommand() API.
 *
 * @details     Reads the current UBX-CFG-RATE navigation rate using a
 *              hand-built ubxPacket, then modifies the measurement interval
 *              and writes it back. sendCommand() returns a
 *              sfe_ublox_status_e enum and populates the custom payload
 *              with the module's response, enabling full round-trip UBX
 *              communication from outside the library.
 *
 * Requirements:
 ...
 * 2) Set NAV_RATE to the desired navigation rate in Hz.
 * 3) Upload the sketch.
 * 4) Open Serial Monitor at 115200 baud.
 *
 * Expected output:
 * - Current measurement rate (Hz), new rate, confirmation message, then
 *   continuous latitude, longitude (degrees * 10^-7), altitude (mm), and
 *   milliseconds printed at the new rate.
 *
 * Notes:
 * - The ubxPacket structure fields are documented inline in the sketch;
 *   refer to the u-blox receiver description for message class/ID values.
 *
