/**
 **************************************************
 * @file        AssistNowOnline_PositionAssist.ino
 * @brief       Fetch AssistNow Online data and provide initial position assistance to the GNSS module.
 *
 * @details     This example uses ESP32 WiFi to obtain AssistNow Online MGA data
 *              from u-blox Thingstream. It demonstrates two ways to provide initial
 *              position assistance: including the approximate position in the
 *              Thingstream HTTP request (define USE_SERVER_ASSISTANCE), or pushing
 *              it directly to the module with setPositionAssistanceLLH. UTC time
 *              assistance is also sent via NTP. MGA-ACK acknowledgements are used
 ...
 * 2) Optionally edit the latitude/longitude/altitude constants near the top
 *    of the sketch to match your approximate location.
 * 3) Connect the u-blox module to your board via I2C.
 * 4) Upload the sketch.
 * 5) Open Serial Monitor at 115200 baud and press any key to begin.
 *
 * Expected output:
 * - WiFi connection, HTTP response, bytes from Thingstream.
