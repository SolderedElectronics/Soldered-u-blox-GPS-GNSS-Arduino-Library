/**
 **************************************************
 * @file        AssistNowOnline_Serial.ino
 * @brief       Fetch AssistNow Online data from u-blox Thingstream over WiFi and push it to a GNSS module via UART.
 *
 * @details     This example uses ESP32 WiFi to perform an HTTP GET request to the
 *              u-blox Thingstream AssistNow Online service. Unlike the standard
 *              AssistNowOnline example, the MGA data is pushed to the GNSS module
 *              over Serial1 (UART) rather than I2C. Optionally, UBX_MGA_ACK_DATA0
 *              acknowledgements can be enabled for more robust data transfer.
 *
 * Requirements:
 * - Board:      ESP32
 * - Hardware:   Any u-blox GNSS module connected via UART (Serial1 at 9600 baud)
 * - Extra:      WiFi
 *
 * How to use:
 * 1) Update secrets.h with your WiFi SSID, password, and AssistNow token.
 * 2) Connect the u-blox module UART TX/RX to Serial1 on your ESP32.
 * 3) Upload the sketch.
