/**
 **************************************************
 * @file        AssistNowOnline_TimeDelay.ino
 * @brief       Fetch AssistNow Online data and push it with separate time assistance to allow reuse within 2-4 hours.
 *
 * @details     AssistNow Online data is valid for 2-4 hours and can therefore be
 *              cached and re-used without downloading it again. However, the time
 *              assistance embedded in the payload must be stripped out and replaced
 *              with fresh time each reuse. This example demonstrates that pattern:
 *              the ESP32 RTC is set from NTP, UTC time is pushed to the module using
 *              setUTCTimeAssistance, and the AssistNow payload is sent without its
 *              embedded time data (the 'true' parameter to pushAssistNowData).
 *
 * Requirements:
 * - Board:      ESP32
 ...
 *
 * Expected output:
 * - WiFi connection, NTP time, HTTP response and bytes from Thingstream.
