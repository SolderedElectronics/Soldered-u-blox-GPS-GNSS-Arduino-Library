/**
 **************************************************
 * @file        AssistNowOffline.ino
 * @brief       Fetch AssistNow Offline data from u-blox Thingstream over WiFi and push it to a GNSS module.
 *
 * @details     This example uses ESP32 WiFi to perform an HTTP GET request to the
 *              u-blox Thingstream AssistNow Offline service. The received MGA-ANO
 *              data is filtered to today's date using findMGAANOForDate, then pushed
 *              over I2C to the GNSS module. Network time (NTP) is used to provide
 *              UTC time assistance. Optionally, UBX_MGA_ACK_DATA0 acknowledgements
 *              can be enabled for more robust data transfer.
 *
 * Requirements:
 * - Board:      ESP32
 ...
 *
 * Expected output:
 * - WiFi and HTTP status, bytes received from Thingstream.
 * - Today's data start/end offsets.
 ...
 * - Always use resolution=1 in the query; findMGAANOForDate requires an
 *   exact date match.
 *
