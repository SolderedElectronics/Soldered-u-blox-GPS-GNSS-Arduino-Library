 ...
 **************************************************
 * @file        TIM_TM2.ino
 * @brief       Log TIM TM2 timing messages to SD card with nanosecond resolution.
 *
 * @details     Configures the u-blox GNSS to send TIM TM2 reports automatically
 *              over I2C whenever a rising or falling edge is detected on the INT
 *              pin, and logs the raw UBX packets to an SD card. A callback prints
 *              edge timestamps with nanosecond resolution to the Serial Monitor.
 *              Connecting the PPS pin to the INT pin with a jumper wire will
 *              produce one TIM TM2 message per second.
 *
 *              Note: TIM TM2 can only capture one rising and one falling edge per
 *              navigation solution. At 1 Hz, only the most recent edges of a
 *              faster signal are visible, though the rising-edge counter still
 *              increments at the full signal rate.
 *
 * Requirements:
 * - Board:      Any Arduino-compatible board with I2C and SPI
 * - Hardware:   u-blox GNSS module (e.g. NEO-M8, ZED-F9P, NEO-M9N)
 * - Extra:      SD card; jumper wire from PPS to INT pin
 *
 * How to use:
 * 1) Connect the u-blox module to your board via I2C.
 * 2) Connect the PPS breakout pin to the INT pin with a jumper wire.
 * 3) Insert a formatted micro-SD card.
 * 4) Upload the sketch.
 * 5) Open Serial Monitor at 115200 baud and press any key to start logging.
 * 6) Press any key again to stop logging and close the file.
 *
 * Expected output:
 * - Falling/rising edge flags, rising edge counter, and sub-millisecond
 *   timestamps (towMsR, towSubMsR, towMsF, towSubMsF) printed per event.
 * - Dots printed while waiting for a TIM TM2 message.
 * - "Logging stopped. Freezing..." when logging ends.
 *
 * Notes:
 * - To minimise I2C bus errors, consider disabling the I2C pull-up resistors
 *   on the u-blox module breakout board.
 *
