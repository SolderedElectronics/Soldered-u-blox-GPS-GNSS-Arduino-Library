/**
 **************************************************
 * @file        OpenLogESP32_SPI_SDIO.ino
 * @brief       Log RXM SFRBX and RAWX data via SPI to SD card using 4-bit SDIO.
 *
 * @details     Configures the ZED-F9P to send RXM SFRBX and RAWX reports
 *              automatically over SPI (up to 20 Hz) and writes the UBX data
 *              to an SD card using the ESP32 SD_MMC (4-bit SDIO) interface.
 *              Callbacks count incoming message groups. The log filename is
 *              derived from the current GPS date/time after a 3D fix is
 *              acquired. Requires the ZED-F9P DSEL jumper to be closed to
 *              select SPI mode.
 *
 * Requirements:
 * - Board:      ESP32-based board with 4-bit SDIO SD support
 * - Hardware:   u-blox ZED-F9P (SPI mode, DSEL jumper closed)
 * - Extra:      SD card (connected via SD_MMC / 4-bit SDIO)
 *
 * How to use:
 * 1) Close the DSEL jumper on the ZED-F9P breakout to enable SPI mode.
 * 2) Wire the ZED-F9P SPI pins to the ESP32 (SCK/PICO/POCI/CS).
 ...
 *
 * Expected output:
 * - SFRBX and RAWX message group counts plus buffer high-tide % every second.
