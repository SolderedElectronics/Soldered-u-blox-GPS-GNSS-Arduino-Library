/**
 **************************************************
 * @file        RXM_SFRBX_and_RAWX.ino
 * @brief       Log RXM SFRBX and RAWX messages to SD card using callbacks.
 *
 * @details     Configures the u-blox GNSS to send RXM SFRBX and RAWX reports
 *              automatically over I2C and logs the UBX data to an SD card.
 *              Callbacks count incoming message groups. Data is buffered in
 *              RAM and flushed to the SD card in 512-byte blocks. Suitable
 *              for Post-Processed Kinematic (PPK) workflows using RTKLIB.
 *
 *              Note: requires a u-blox ADR, High Precision GNSS, or Time Sync
 *              product (e.g. ZED-F9P, NEO-M8P-2) and a processor with
 *              sufficient RAM for the 16 KB file buffer.
 *
 * Requirements:
 * - Board:      Any Arduino-compatible board with I2C, SPI, and ample RAM
 * - Hardware:   u-blox High Precision GNSS module (e.g. ZED-F9P, NEO-M8P-2)
 * - Extra:      SD card
 *
 * How to use:
 * 1) Connect the u-blox module to your board via I2C.
 * 2) Insert a formatted micro-SD card.
 * 3) Upload the sketch.
 * 4) Open Serial Monitor at 115200 baud and press any key to start logging.
 * 5) Press any key again to stop logging and close the file.
 *
 * Expected output:
 * - SFRBX and RAWX message group counts printed once per second.
 * - Warning if the file buffer exceeded 80% capacity.
 ...
 * - RAWX data can legitimately contain 0x7F, so UBX7F checking is disabled.
 * - Data can be analyzed with u-center or processed with RTKLIB.
 *
