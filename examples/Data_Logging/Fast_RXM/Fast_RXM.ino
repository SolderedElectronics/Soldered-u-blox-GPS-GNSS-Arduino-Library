 ...
 **************************************************
 * @file        Fast_RXM.ino
 * @brief       Log RXM SFRBX and RAWX data to SD card at maximum rate without callbacks.
 *
 * @details     Configures the u-blox GNSS to send RXM SFRBX and RAWX reports
 *              automatically over I2C and logs them to an SD card in UBX format
 *              without using callbacks, running as fast as the module allows.
 *              Only GPS is enabled and other constellations are disabled to
 *              achieve up to 25 Hz navigation rate. Data can be replayed and
 *              analyzed with u-center or post-processed with RTKLIB.
 *
 *              Note: this example requires a u-blox ADR, High Precision GNSS,
 *              or Time Sync product (e.g. ZED-F9P, NEO-M8P-2).
 *
 * Requirements:
 ...
 *
 * Expected output:
 * - Bytes written to SD card reported once per second.
 ...
 * - Data can be analyzed with u-center or RTKLIB.
 *
