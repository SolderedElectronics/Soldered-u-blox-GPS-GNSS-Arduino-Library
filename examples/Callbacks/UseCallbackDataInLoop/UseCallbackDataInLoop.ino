/**
 **************************************************
 * @file        UseCallbackDataInLoop.ino
 * @brief       Access NAV-PVT callback data from the main loop using the callbackCopyValid flag.
 *
 * @details     This example demonstrates how to use the library's internal
 *              callbackCopyValid flag to check for new NAV-PVT data in the main loop
 *              rather than relying solely on the callback function. A NAV-PVT callback
 *              is registered, but the loop manually inspects and clears
 *              packetUBXNAVPVT->automaticFlags.flags.bits.callbackCopyValid to read
 *              and print the time, latitude, longitude and altitude directly. Clearing
 *              the flag prevents the callback from also firing.
 *
 * Requirements:
 ...
 *
 * Expected output:
 * - UTC time, latitude and longitude (degrees * 10^-7) and height above MSL (mm)
 *   printed at 2 Hz from the main loop.
 *
