 ...
 **************************************************
 * @file        ESF.ino
 * @brief       Read External Sensor Fusion (ESF) data from a NEO-M8U using callbacks.
 *
 * @details     This example configures the ESF messages on the NEO-M8U and registers
 *              callbacks for ESF-ALG (attitude), ESF-INS (inertial navigation),
 *              ESF-MEAS (sensor measurements) and ESF-STATUS (fusion status).
 *              Data is processed automatically without polling.
 *
 * Requirements:
 ...
 *
 * Expected output:
 * - Roll, pitch and yaw in degrees from ESF-ALG each second.
 * - X/Y/Z accelerations from ESF-INS.
 * - Sensor type and data field for each measurement in ESF-MEAS.
 * - Fusion mode and per-sensor calibration status from ESF-STATUS.
 *
 * Notes:
