 ...
 **************************************************
 * @file        NAV_SVIN.ino
 * @brief       Monitor a Survey-In operation using NAV-SVIN callback on a ZED-F9P base station.
 *
 * @details     This example registers a NAV-SVIN callback to monitor a Survey-In
 *              (SVIN) operation used to establish a precise base station position.
 *              Press any key in Serial Monitor to start a Survey-In (60 s minimum
 *              duration, 5.0 m accuracy). The callback prints SVIN active/valid
 *              status, elapsed time, ECEF position with high-precision offset, and
 ...
 *
 * How to use:
 * 1) Connect the ZED-F9P to your board via I2C.
 * 2) Upload the sketch.
 * 3) Open Serial Monitor at 115200 baud.
 * 4) Press any key to start the Survey-In.
 *
 * Expected output:
 * - SVIN active/valid status, observation time (s), ECEF X/Y/Z position (cm)
 *   and mean accuracy (cm) printed each navigation cycle during Survey-In.
 *
