/**
 **************************************************
 * @file        ESF_RAW_In_Loop.ino
 * @brief       Access ESF RAW IMU data at 100 Hz in the main loop without using the callback.
 *
 * @details     This example configures ESF-RAW automatic messages on the NEO-M8U or
 *              ZED-F9R. A callback is registered but the main loop manually checks
 *              callbackCopyValid to read and decode the raw IMU data (gyroscope,
 *              accelerometer, temperature) directly. Manually clearing the flag
 *              prevents the callback from firing. Only the first seven readings per
 *              message are printed by default.
 *
 * Requirements:
