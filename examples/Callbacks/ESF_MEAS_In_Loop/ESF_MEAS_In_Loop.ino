/**
 **************************************************
 * @file        ESF_MEAS_In_Loop.ino
 * @brief       Access ESF MEAS (wheel ticks and IMU) data in the main loop without using the callback.
 *
 * @details     This example configures ESF-MEAS automatic messages on the NEO-M8U
 *              or ZED-F9R. A callback is registered but the main loop manually checks
 *              the callbackCopyValid flag to read and decode sensor data (wheel ticks,
 *              speed, gyroscope, accelerometer and temperature) directly, bypassing the
 *              callback. Manually clearing the flag prevents the callback from firing.
 *
 * Requirements:
 * - Board:      Any Arduino-compatible board with I2C (400 kHz recommended)
 * - Hardware:   u-blox NEO-M8U or ZED-F9R
 *
 * How to use:
 * 1) Connect the module to your board via I2C.
 * 2) Upload the sketch.
 * 3) Open Serial Monitor at 230400 baud.
 *
 * Expected output:
 * - Time tag, sensor type labels and decoded sensor values printed each ESF-MEAS
 *   message.
 *
 * Notes:
 * - Serial must run at 230400 baud or faster; ESF-MEAS can arrive at high rates.
